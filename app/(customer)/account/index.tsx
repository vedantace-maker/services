...
// ── Type meta (same as location.tsx) ─────────────────────────────────────────
const TYPE_META = {
...
export default function CustomerAccount() {
    ...
    const { profiles, activeProfile, setActiveProfile } = useLocationStore();
    ...
    const handleDetectLocation = async () => {
        ...
        try {
            ...
            if (results.length > 0) {
                const r = results[0];
                setAddress(
                    [r.name, r.street, r.district, r.city, r.region, r.postalCode]
                        .filter(Boolean).join(', ')
                );
            } else {
    ...
    const MENU_SECTIONS = [
        {
            ...
            items: [
                ...
                { icon: 'car-sport-outline' as const, label: 'My Vehicles', sub: 'Manage your bikes & scooties', route: '/(customer)/account/my-vehicles', color: '#7C3AED' },
                { icon: 'gift-outline' as const, label: 'Refer & Earn', sub: 'Invite friends and earn rewards', route: '/(customer)/account/refer-earn', color: '#D97706' },
            ],
        },
        {
            title: 'More',
            items: [
                ...
                { icon: 'information-circle-outline' as const, label: 'About MotoBee', sub: 'Version 1.0.0', route: '/(customer)/account/about' as any, color: Colors.textTertiary },
            ],
        },
    ];
            ...
            >
                ...
                <View style={styles.locationSection}>
                    ...
                    <View style={styles.locationSectionHeader}>
                        <View style={styles.locationSectionLeft}>
                            <Ionicons name="location-outline" size={16} color={Colors.primary} />
                            <Text style={styles.locationSectionTitle}>DELIVERY LOCATIONS</Text>
                        </View>
                        <TouchableOpacity
                            style={styles.manageBtn}
                            onPress={() => router.push('/(customer)/account/location' as any)}
                            activeOpacity={0.8}
                        >
                    ...
                    {/* Active delivery address highlight */}
                    {activeProfile ? (
                        <View style={styles.activeAddressCard}>
                            <View style={[
                                styles.activeAddressIcon,
                                { backgroundColor: TYPE_META[activeProfile.type].bg },
                            ]}>
                                <Ionicons
                                    name={TYPE_META[activeProfile.type].icon as any}
                                    size={18}
                                    color={TYPE_META[activeProfile.type].color}
                                />
                            </View>
                            <View style={styles.activeAddressBody}>
                                <View style={styles.activeAddressTopRow}>
                                    <Text style={styles.activeAddressLabel}>{activeProfile.label}</Text>
                        ...
                        >
                            ...
                            <Text style={styles.noLocationText}>No active delivery location</Text>
                            <Ionicons name="chevron-forward" size={14} color={Colors.textTertiary} />
                        </TouchableOpacity>
                    )}

                    {/* Switch location — other saved profiles */}
                    {profiles.length > 1 && (
                        <>
                    ...
                    {/* If only 1 or 0 profiles — show add button */}
                    {profiles.length <= 1 && (
                        <TouchableOpacity
...
const styles = StyleSheet.create({
    ...
    editBtn: {
        ...
        borderRadius: Radius.md, backgroundColor: Colors.primary + '18',
    ...
    fieldLabel: {
        ...
        fontWeight: '800', letterSpacing: 0.8, paddingLeft: 2,
    },
    inputRow: {
        flexDirection: 'row', alignItems: 'center', gap: Spacing.sm,
    ...
    detectBtn: {
        ...
        borderWidth: 1, borderColor: Colors.primary, marginTop: 6,
    },
    ...
    activeAddressText: {
        ...Typography.caption, color: Colors.textSecondary, lineHeight: 17,
    },
    ...
    noLocationText: {
        ...Typography.body, color: Colors.textTertiary, flex: 1,
    },
    ...
    menuSectionTitle: {
        ...Typography.overline, color: Colors.textTertiary, fontWeight: '700',
